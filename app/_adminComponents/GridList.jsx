...
export default function GridList({ list, grid, handleList, handleGrid }) {
  return (
    <div className="flex items-center justify-end gap-2 p-4">
      <button
        className={`rounded-sm p-1 duration-300 ${list ? "scale-110 bg-indigo-600" : "scale-100 bg-transparent"}`}
        onClick={handleList}
      >
      ...
      <button
        onClick={handleGrid}
        className={`rounded-sm p-1 duration-300 ${grid ? "scale-110 bg-indigo-600" : "scale-100 bg-transparent"}`}
      >
