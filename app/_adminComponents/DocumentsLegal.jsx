...
export default function DocumentsLegal() {
  ...
  async function handleFileUpload(e) {
    ...
    if (file?.size > 2000000) return toast.error("File must me less than 2 mb");
    ...
    if (res?.error) {
      toast.error(`Upload failed for ${fieldName}:`, res.error);
      return;
    }

    // ✅ Save uploaded URL to formData
    setFormData((prev) => ({
      ...prev,
  ...
  async function handleDeleteFile(fieldName) {
    ...
    // Extract file path from URL (the part after the bucket name)
